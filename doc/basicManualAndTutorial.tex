...
artvisc 	& Artificial viscosity (\textit{mu\_art}) & save artvisc with solution = .true. \\ \hline
\end{longtable}

\textbf{Watch out for \textit{artvisc}:} unlike \textit{u\_tau}/\textit{u\_tau\_vector}/\textit{turb}, there is no dedicated \textit{surface save artvisc} keyword --- surface saving reuses the \textbf{general, volume-wide} \textit{save artvisc with solution} flag. If that flag is \textit{.true.} in the simulation control file (even if you only care about volume output), every saved surface face gets an extra trailing value, and you \textbf{must} add \textit{artvisc} to \textit{additional variables} in \textit{horses2plt} to keep the reads in sync, even if you never request \textit{mu\_art} as an output variable. Forgetting it causes an ``Array dimensions mismatch'' desync, typically surfacing on the \textbf{second} element (the first element's own reads still land on the right bytes; the leftover unread value only breaks the read from there on).
...
\item \textbf{tau}: wall shear stress $\tau_w = \rho\,u_\tau^2\,\text{sign}(u_\tau)$ (stress units). Requires \textit{u\_tau}.
\item \textbf{tau\_x, tau\_y, tau\_z}: Cartesian components of the wall-shear-stress vector, $\tau_i = \rho\,|\vec{u}_\tau|\,u_{\tau,i}$ (stress units), i.e. the vectorial counterpart of \textit{tau}, pointing in the same direction as the friction-velocity vector. Requires the \textit{u\_tau\_vector} additional variable (no separate solver-side flag is needed beyond what \textit{u\_tau\_x/y/z} already requires).
...
Statistics files generate the following variables (being $R_{ij}$ the components of the Reynolds Stress tensor):
...
As with standard solution files, the variables to export can be restricted with \textit{output variables}. The following shorthands are also accepted:

\begin{itemize}
\item \textbf{Vmean} or \textbf{V}: expands to \textit{Umean, Vmean, Wmean}.
\item \textbf{Rij}: expands to \textit{Rxx, Ryy, Rzz, Rxy, Rxz, Ryz}.
\item \textbf{Fij}: expands to \textit{Fxx, Fyy, Fzz, Fxy, Fxz, Fyz}.
\item \textbf{all}: every Reynolds and (if available) Favre variable.
\end{itemize}
